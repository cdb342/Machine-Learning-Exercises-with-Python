...
Return:数据的类别
"""
class Perceptron_3Classes_Adam():
    ...
    def fit(self,X,y):
        ...
        self.omiga=[np.random.randn(self.X.shape[1],3)]#按照高斯分别随机初始化权值
        self.cost=[]
        ...
        for i in range(self.times):
            for j in range(Num_batch):
                ...
                term1=z[(index0, H_omiga[j * 16:j * 16 + 16].astype(int))]#
                term2=z[(index0,self.y[j * 16:j * 16 + 16].astype(int))]#
                ...
                g2=np.dot(self.X[j * 16:j * 16 + 16].T,np.where(H_omiga[j*16:j*16+16]==2,1,0) - np.where(self.y[j*16:j*16+16]==2,1,0 ))
                v2 = beta1 * v2 + (1 - beta1) * g1
...
aa=Perceptron_3Classes_Adam(0.1,300)
...
feature1=np.linspace(0,5,100)#在0-65间均匀生成100个数
feature2=np.linspace(0,3,100)
XX1,XX2=np.meshgrid(feature1,feature2)
XX=np.insert(np.c_[XX1.ravel(),XX2.ravel()], 0, 1, axis=1)
z = np.dot(XX, aa.omiga[0])
yy = np.argmax(z, axis=1).reshape(np.shape(XX1))
...
def animate(i):#定义动画更新函数
    ...
    yy = np.argmax(z, axis=1).reshape(np.shape(XX1))
    cont=ax[0].contourf(XX1,XX2,yy,alpha=0.2,cmap='Set3')
...
ani=animation.FuncAnimation(fig,animate,frames=1200,interval=10)
plt.legend()
plt.show()
