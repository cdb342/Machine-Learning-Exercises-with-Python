...
class LogisticRegression_SGD():
    def __init__(self,alpha,times):#参数为学习率与梯度下降迭代次数
        ...
        self.times=times
    def fit(self,X,y):
        np.random.seed(2)#使特征集与标签集以相同的顺序打乱
        self.X=np.random.permutation(np.insert(X,0,1,axis=1))#在特征集第一列插入一列全1的数据，并打乱特征集顺序以加快随机梯度下降速度
        np.random.seed(2)#使特征集与标签集以相同的顺序打乱
        self.y=np.random.permutation(y.reshape(-1,1))#将标签集调整为列向量,并打乱顺序以加快随机梯度下降速度
        np.random.seed(6)#使每次生成的随机数相同，确保每次运行有相同的结果
        self.theta=[np.random.rand(self.X.shape[1],1)]#初始化特征集的权值
        self.cost=[]
        z = np.dot(self.X, self.theta[0])  # 计算特征集与权值的乘积
        h_theta = sigmoid(z)  # 计算特征集属于标签1的概率
        len_X=self.X.shape[0]
        for i in range(self.times):#运行times次梯度下降
            for j in range(len_X):
                self.index_theta=i*len_X+j#每次迭代的权值索引
                z[j]=np.dot(self.X[j],self.theta[self.index_theta])
                h_theta[j]=sigmoid(z[j])#用更新的权值计算本次迭代的样本属于标签1的概率
                error = self.y[j] - h_theta[j]#计算标签与预测值的差
                J = -np.sum(self.y * np.log(h_theta) + (1 - self.y) * np.log(1 - h_theta))  #计算整体的损失函数(每次只改变一个样本值)
                self.cost.append(J)
                beta=self.alpha/(i*self.index_theta*0.01+0.2)#学习率逐次衰减，加快学习速率
                self.theta.append(self.theta[self.index_theta] + beta * error*self.X[j].reshape(-1,1))# 梯度下降更新权值并将每一次迭代计算出的权值储存在数组中，方便后续可视化
...
aa = LogisticRegression_SGD(0.1,6)
...
ax1=plt.subplot(132,xlim=(0, ite), ylim=(24, 50))#创建第二个坐标轴，绘制损失函数下降情况
...
plt.xlabel("Iteration")
plt.ylabel("Accuracy")
plt.title("Logistic Regression-Stochastic Gradient Descent Accuracy Change")
...
def animate(i):#动画更新函数
    exam2 = -(aa.theta[i + 1][0] + aa.theta[i + 1][1] * (exam1 - s_train.mean) / s_train.std) / aa.theta[i + 1][2] * s_train.std + s_train.mean#计算exam2的值随exam1的变化，由于优化权值时用的是标准化后的数据，此时需要反标准化
...
anim = animation.FuncAnimation(fig, animate,frames=ite, interval=200, blit=True)#创建动画
plt.legend()
plt.show()
#anim.save('LogisticRegression_SGD.gif',writer='imagemagick',fps=30)#保持动态图（需要安装imagemagick）