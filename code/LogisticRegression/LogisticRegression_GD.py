...
class LogisticRegression_GD():
    ...
    def fit(self,X,y):
        ...
        self.y=y.reshape((-1,1))#将标签集调整为列向量
        ...
        for i in range(self.times):#运行times次梯度下降
            ...
            error=self.y-h_theta#计算标签与预测值的差
            J=-np.sum(self.y*np.log(h_theta)+(1-self.y)*np.log(1-h_theta))#计算损失函数
            self.cost.append(J)
...
aa = LogisticRegression_GD(0.01, 50)
...
ax1=plt.subplot(132,xlim=(0, 50), ylim=(25, 50))#创建第二个坐标轴，绘制损失函数下降情况
plt.xlabel("Iteration")
plt.ylabel("Cost")
plt.title("Cost Function Change")
ax2=plt.subplot(133,xlim=(0, 50), ylim=(0, 1))#创建第三个坐标轴，绘制准确率变化情况
plt.xlabel("Iteration")
plt.ylabel("Accuracy")
plt.title("Logistic Regression-Gradient Descent Accuracy Change")
...
def init():#初始化函数
    ...
    line3.set_data([[], []])
    return line0,line1,line2,line3
anim = animation.FuncAnimation(fig, animate, init_func=init,frames=50, interval=200, blit=True)#创建动画
plt.legend()
plt.show()
#anim.save('LogisticRegression_GD.gif',writer='imagemagick',fps=30)#保持动态图（需要安装imagemagick）
