...
class Multinomial_Naive_Bayesian():
    ...
    def conditional_probability(self):
        ...
        for label in self.label_set:
            cond_pro[self.label_set.index(label)] = 1+np.sum(np.array(training_text_vector)[[k for k in range(len(self.training_label)) if self.training_label[k] == label]],axis=0)
            cond_pro[self.label_set.index(label)] = cond_pro[self.label_set.index(label)]/np.sum(cond_pro[self.label_set.index(label)])
        return  cond_pro
    def predict(self,text):
        ...
        for i in range(len(result)):
            for j in range(len(self.label_set)):
                tem[j] = cond_pro[j] ** np.array(text_vector)[i]
